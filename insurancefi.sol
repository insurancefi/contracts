...
contract InsuranceFi is Context, IBEP20, Ownable {
    ...
    constructor () {
        // set pool-address
        insuranceFundAddress = payable(0x0Ac93363ED5d73dA3fD4931D9c3EBd7Ad47B4e9A);
        ...
        // create pancake-pair (main-net: 0x10ED43C718714eb63d5aA57B78B54704E256024E, test-net:0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3 )
        IPancakeswapV2Router02 _pancakeswapV2Router = IPancakeswapV2Router02(0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3); // test-net
        pancakeswapV2Pair = IPancakeswapV2Factory(_pancakeswapV2Router.factory()).createPair(address(this), _pancakeswapV2Router.WETH());
        address pairBusd = IPancakeswapV2Factory(_pancakeswapV2Router.factory()).createPair(address(this), busdToken);
        ...
        IBEP20(busdToken).approve(address(pancakeswapV2Router), ~uint256(0));
        IBEP20(busdToken).approve(address(pairBusd), ~uint256(0));
        IBEP20(busdToken).approve(address(this), ~uint256(0));
    ...
    function setPoolsAddress(address pr, address dao, address charity) external onlyOwner {
    ...
    function setFees(uint256 _busdFee, uint256 _prFee, uint256 _insuranceFundFee, uint256 _buyFee) external onlyOwner {
    ...
    function _getSellFeeValues(uint256 tAmount) private view returns (uint256) {
        ...
        uint256 fee = tAmount.mul(insuranceFundFee + busdPoolFee + prAndDevFee).div(10**2);
        uint256 tTransferAmount = tAmount.sub(fee);
        return tTransferAmount;
    }
    ...
    function swapAndCharge(uint256 tokenBalance) private lockTheSwap {
        ...
        uint256 _currentFee = insuranceFundFee.add(busdPoolFee).add(prAndDevFee);

        (bool success, ) = payable(insuranceFundAddress).call{
        value: currentBalance.mul(insuranceFundFee).div(
            _currentFee
        ),
        gas: 30000
        }("");
        ( success, ) = payable(prPoolAddress).call{
        value: currentBalance.mul(prAndDevFee).div(
            _currentFee
        ),
        gas: 30000
        }("");
        ( success, ) = payable(busdPoolAddress).call{
        value: currentBalance.mul(busdPoolFee).div(
            _currentFee
        ),
        gas: 30000
        }("");
    ...
    event UpdateFees(uint256 busdFee, uint256 prFee, uint256 insuranceFundFee ,  uint256 buyFee);
